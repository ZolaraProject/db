...
sudo docker run --name postgresqldiff -e POSTGRES_PASSWORD=zolara_changeme -p 5443:5432 -d postgres:16.3 > /dev/null

echo "Waiting for PostgreSQL to be ready..."
until PGPASSWORD=zolara_changeme sudo docker exec -it postgresqldiff psql -U postgres -c '\q' 2> /dev/null; do sleep 1; done
...
# Generate the diff
sudo docker run -v ${PWD}:/liquibase/changelog -v ${PWD}/out:/out --network host liquibase/liquibase --defaultsFile=/liquibase/changelog/$chosen_property --referenceUrl="jdbc:postgresql://127.0.0.1:5443/zolara" --referenceUsername=postgres  --referencePassword=zolara_changeme --changeLogFile=/out/diff.sql diffChangeLog
